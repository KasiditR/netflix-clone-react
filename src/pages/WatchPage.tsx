...
const WatchPage = () => {
    ...
    const getTrailers = async () => {
      try {
        const response = await axiosInstance.get(
          `/api/v1/content/trailers?id=${id}&contentType=${contentType}`
        );
        setTrailers(response.data.contents);
      } catch (error: any) {
  ...
  useEffect(() => {
    const getSimilarContent = async () => {
      try {
        const response = await axiosInstance.get(
          `/api/v1/content/similar?id=${id}&contentType=${contentType}`
        );
        setSimilarContent(response.data.contents);
      } catch (error: any) {
        if (error.message.includes('404')) {
          setSimilarContent([]);
        }
  ...
  useEffect(() => {
    const getContentDetails = async () => {
      try {
        const response = await axiosInstance.get(
           `/api/v1/content/detail?id=${id}&contentType=${contentType}`
        );
